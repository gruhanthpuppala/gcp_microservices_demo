...
1.  Create a Google Kubernetes Engine cluster and make sure `kubectl` is pointing
    ...
    ```sh
    gcloud container clusters create-auto demo --region=us-central1-a
    ```
...
3.  In the root of this repository, run `skaffold run --default-repo=us-central1-a-docker.pkg.dev/[PROJECT_ID]/microservices-demo`,
    ...
    **Troubleshooting:** If you get "No space left on device" error on Google
    ...
    then run `skaffold run -p gcb --default-repo=us-central1-a-docker.pkg.dev/[PROJECT_ID]/microservices-demo` instead.
